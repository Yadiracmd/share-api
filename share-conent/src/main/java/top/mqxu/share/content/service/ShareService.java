...
import top.mqxu.share.content.mapper.UserMapper;
...
public class ShareService {
    ...
    public List<Share> getList(String title, Integer pageNo, Integer pageSize, Long userId) {
        ...
        // 2. 如果用户登录了，name查询 mid_user_share 表
        else {
            sharesDeal = shares.stream()
                    .peek(share -> {
                        MidUserShare midUserShare = midUserShareMapper.selectOne(
                                new QueryWrapper<MidUserShare>()
                                        .lambda()
    ...
    /**
     * 投稿 * * @param shareRequestDTO 投稿参数 * @return int
     */
    public int contribute(ShareRequestDTO shareRequestDTO) {
        Share share = Share.builder().isOriginal(shareRequestDTO.getIsOriginal()).author(shareRequestDTO.getAuthor()).price(shareRequestDTO.getPrice()).downloadUrl(shareRequestDTO.getDownloadUrl()).summary(shareRequestDTO.getSummary()).buyCount(0).title(shareRequestDTO.getTitle()).userId(shareRequestDTO.getUserId()).cover(shareRequestDTO.getCover()).showFlag(false).auditStatus("NOT_YET").reason("未审核").createTime(new Date()).updateTime(new Date()).build();
    ...
    public Long approveShare(Long id) {
    ...
    public void addBouns(Long userId, int points) {
        User user = userMapper.selectById(userId);
        if (user != null) {
            user.setBonus(user.getBonus() + points);
